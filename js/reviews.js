...
(function(){
  ...
  function render(d){
    ...
    document.getElementById('reviewsRating').innerHTML =
      ...
      '<span class="reviews__count">'+(d.total>=10?d.total+' avaliações no Google':'Avaliação no Google')+'</span>';
    var cards = (d.reviews||[]).filter(function(rv){return (rv.text||'').trim();}).map(function(rv){
    ...
    document.getElementById('reviewsList').innerHTML = '<div class="reviews__track">'+cards+'<span class="reviews__dup" aria-hidden="true">'+cards+'</span></div>';
    var hero=document.getElementById('heroRating');
    if(hero){ hero.innerHTML='<i class="ri-google-fill"></i><strong>'+d.rating.toFixed(1).replace('.',',')+'</strong><span class="hero__rating-stars">'+stars(d.rating)+'</span><span class="hero__rating-label">'+(d.total>=10?'no Google \u00b7 '+d.total+' avalia\u00e7\u00f5es':'Avalia\u00e7\u00e3o no Google')+'</span>'; if(d.url) hero.href=d.url; hero.hidden=false; }
